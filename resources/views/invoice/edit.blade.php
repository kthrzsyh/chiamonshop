<x-app-layout>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <x-text-input id="customer_name" name="customer_name" type="text"
                                class="mt-1 block w-full" value="{{ $invoice->customer_name }}" required />
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-5 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <input type="number" name="items[{{ $index }}][price]"
                                            value="{{ $item->price }}" class="w-full border-gray-300 rounded-lg"
                                            onchange="calculateSubtotal({{ $index }})" step="0.01">
                                    </div>
                                    <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
                        <div>
                            ...
                            <x-text-input id="shipping_cost" name="shipping_cost" type="number"
                                class="mt-1 block w-full" value="{{ $invoice->shipping_cost ?? 0 }}" step="0.01" />
                        ...
                        <div>
                            ...
                            <x-text-input id="box_fee" name="box_fee" type="number" class="mt-1 block w-full"
                                value="{{ $invoice->box_fee ?? 0 }}" step="0.01" />
                            <x-input-error class="mt-2" :messages="$errors->get('box_fee')" />
                        </div>
                    </div>

                    <div class="mb-4">
                        <x-input-label for="notes" :value="__('Catatan (opsional)')" />
                        <textarea name="notes" id="notes" rows="3" class="w-full border-gray-300 rounded">{{ $invoice->notes }}</textarea>
        ...
        <div class="p-3 border rounded-md bg-gray-50">
            <div class="grid grid-cols-1 sm:grid-cols-5 gap-2">
                ...
                <div>
                    ...
                    <input type="number" name="items[${index}][price]" class="w-full border-gray-300 rounded" step="0.01" onchange="calculateSubtotal(${index})">
                </div>
                <div>
    ...
    `;
        ...
        function removeRow(button) {
            button.closest('.p-3.border.rounded-md, .p-4.border.rounded-lg').remove();
        }
        ...
        function calculateSubtotal(rowIndex) {
            ...
            if (subtotalInput) subtotalInput.value = (qty * price).toFixed(2);
        }
    </script>
